...
test("default voice payload matches previously working native WeChat voice bubble shape", () => {
  const payload = runSender(["--voice-payload-test"]);
...
test("voice send uses previous native SILK bubble payload shape by default", async () => {
  const calls = [];
  ...
  try {
    ...
    assert.equal(payload.bits_per_sample, undefined);
    ...
    assert.equal(uploadReq.base_info.channel_version, "branchwhisper-bridge");
    ...
    assert.equal("bits_per_sample" in item.voice_item, false);
    assert.equal("mid_size" in item.voice_item, false);
    assert.equal("voice_size" in item.voice_item, false);
    assert.equal(item.voice_item.media.encrypted_query_param, "cdn-short-token");
    assert.equal(item.voice_item.media.aeskey, item.voice_item.media.aes_key);
  } finally {
