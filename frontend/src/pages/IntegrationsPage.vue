...
import { Check, CircleAlert, Eraser, PackagePlus, Play, Plus, QrCode, RefreshCw, RotateCw, Save, Send, Square, Trash2, Volume2 } from "@lucide/vue";
    ...
    <div class="ops-page integrations-page">
      <section class="page-head">
        <div>
          ...
          <h1>接入管理</h1>
          <small>微信桥接、扫码登录、语音与表情自检。当前 {{ integrations.summary }}</small>
        </div>
        ...
        <div class="integration-list">
          ...
          >
            ...
            <div class="integration-meta">
              <span class="meta-cell"><span>启用</span><strong>{{ item.enabled ? "自动启用" : "手动" }}</strong></span>
              ...
              <span class="meta-cell"><span>守护</span><strong>{{ item.runtime?.watchdog ? "自动恢复" : "--" }}</strong></span>
            ...
            <p class="integration-state-note" :class="statusClass(item.status)">
              {{ item.last_error || item.runtime?.last_error || (item.status === "running" ? "桥接进程运行中，微信消息会直接进入 BranchWhisper。" : "首次使用先扫码登录。") }}
            </p>
        ...
        <aside class="integration-side">
          <section class="integration-panel">
            ...
            <div class="form-grid compact">
              <label><span>实例 ID</span><input v-model="integrations.form.id" /></label>
              <label><span>微信聊天命名</span><input v-model="integrations.form.chat_name" /></label>
              ...
              <label class="wide"><span>语音触发词</span><textarea v-model="integrations.form.voice_trigger_keywords" rows="4"></textarea></label>
            </div>
          </section>
