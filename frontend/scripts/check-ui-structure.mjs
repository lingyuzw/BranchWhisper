...
assert(diagnosticsPage.includes("diagnostic-check-meta"), "运行诊断检查项需要显示解析目标、解析基准和存在状态");
assert(diagnosticsPage.includes("checkMetadataRows"), "运行诊断页需要用统一 helper 渲染 metadata，避免模板硬编码字段漂移");
assert(diagnosticsPage.includes("raw_target") && diagnosticsPage.includes("resolved_target") && diagnosticsPage.includes("resolution_base"), "运行诊断页需要读取后端 resolved target metadata");
